...
try:
    ...
    _SCRAPER_DEPS = True
except ImportError as _scraper_dep_err:
    _SCRAPER_DEPS = False
...
def _esg_grade_val(val):
    """Parse ESG grade string to a single letter grade."""
...
_DSI: dict = {}           # lid → entry (see _build_description_index)
_DSI_words: list = []     # sorted unique word list for prefix bisect
_DSI_word_idx: dict = {}  # word → frozenset(lids)

# Fully-delisted listings index
_DSI_DEL: dict = {}
_DSI_DEL_words: list = []
...
def _build_description_index():
    ...
    if _fully_del_lids:
        ...
        for lid_val, sub in df[df["listing_id"].isin(_fully_del_lids)].groupby("listing_id"):
            lid  = int(lid_val)
            # Use the last-seen (highest period_ord) non-latest rows for stats
...
# ── Floor plan LLM analysis cache ─────────────────────────────────────────
_FP_LLM_CACHE: dict = {}
_FP_LLM_CACHE_FILE  = os.path.join(os.path.dirname(__file__), "floorplan_llm_cache.json")

def _reload_llm_cache():
    global _FP_LLM_CACHE
    try:
        if os.path.exists(_FP_LLM_CACHE_FILE):
            with open(_FP_LLM_CACHE_FILE, "r", encoding="utf-8") as _f:
                _FP_LLM_CACHE = json.load(_f)
    except Exception:
        pass

_reload_llm_cache()

@app.get("/listing/floorplan/llm")
def floorplan_llm_data(url: str = Query(...)):
    """Return LLM-extracted floor plan analysis from cache (pre-computed by analyze_floorplans_llm.py)."""
    _reload_llm_cache()
    if url in _FP_LLM_CACHE:
        return safe_json(_FP_LLM_CACHE[url])
    return safe_json({"error": "not_cached", "message": "Run analyze_floorplans_llm.py to generate data for this URL"})
...
def floorplan_extract_areas(url: str = Query(...)):
    ...
    try:
        ...
        pil = _PILImage.open(io.BytesIO(resp.content)).convert("L")  # grayscale
        w, h = pil.size
        pil = pil.resize((w * 2, h * 2), _PILImage.LANCZOS)          # 2× upscale
        pil = _IE.Contrast(pil).enhance(2.0)                          # boost contrast
        pil = _IE.Sharpness(pil).enhance(2.0)                         # sharpen
    ...
    # Find dimension pairs (e.g. 3,20 x 4,15)
    dim_items = []
    for it in items:
    ...
    # Find room-name items (known vocab → English)
    room_items = []
    for it in items:
    ...
    for meas in all_measurements:
        # Priority 1: recognised room word → English translation
        room_label = None
        if room_items:
        ...
        # Priority 2: any adjacent non-measurement text (captures labels like
        # "SUPERFICIE ÚTIL", "SALON", room identifiers, dimension prefixes)
        nearby_label = None
        if not room_label:
            def _ok_label(it):
                t = it["text"].strip()
            ...
            # Same-line: no distance cap (handles wide table layouts)
            same_line = [it for it in items if _ok_label(it) and abs(it["cy"] - meas["cy"]) < 25]
            # Other lines: 500px cap
        ...
        if meas["type"] == "area":
            ...
            rows.append({
                "room": room_label,
                "label": nearby_label,
                "area_m2": meas.get("area_calc"),
                "dimensions": meas["dim"],
            })

    # Deduplicate rows with same room + very close area values
    seen = set()
    deduped = []
    for r in rows:
...
# ══════════════════════════════════════════════════════════════════════════
_DESC_SEARCH_CACHE: dict = {}   # {cache_key: response_dict}
_DESC_SEARCH_CACHE_MAX = 500    # evict all when limit reached
...
def description_search_endpoint(
    ...
    # Normalise comma-separated terms so "pool,terrace" and "terrace,pool" share a cache entry
    ...
    def _filter_entry(e):
        """Return True if this index entry passes all query filters."""
    ...
    def _search_index(dsi, dsi_words, dsi_word_idx):
        """Use the pre-built inverted index to find and score matching listings."""
        # Collect candidates via exact + prefix lookup
    ...
    # ── Aggregated unit/house type stats across matched active listings ──
...
def _compute_and_run_expired(province: str, month_name: str, month_abbr: str, month: int, year: int, dev_type: str, _sc):
    ...
    # Load previous month data from the main province sheet in data/
    candidates = [
        f for f in glob.glob(os.path.join(_DATA_DIR, "*.xlsx"))
        if province.lower() in os.path.basename(f).lower()
        and "expired" not in os.path.basename(f).lower()
...
_scraper_state: dict = {
    ...
    "error":             None,
    "aborted":           False,
    "step_times":        {},   # "prov:step_id" -> {start, start_ts, end, end_ts}
    "dd_refreshes":      [],   # [{time, ts, context, prov}]
